...
if os.path.exists("background.mp4"):
    with open("background.mp4", "rb") as f:
        video_src = base64.b64encode(f.read()).decode()
...
.vid-bg { position:fixed;top:0;left:0;width:100%;height:100%;object-fit:cover;z-index:0;filter:brightness(0.4); }
.dark-overlay { position:fixed;top:0;left:0;width:100%;height:100%;background:linear-gradient(135deg,rgba(10,5,40,0.65),rgba(5,10,60,0.55));z-index:1; }
.stars-bg { position:fixed;top:0;left:0;right:0;bottom:0;background:#020818;background-image:radial-gradient(1px 1px at 10% 15%,rgba(255,255,255,0.9) 0%,transparent 100%),radial-gradient(1px 1px at 30% 40%,rgba(255,255,255,0.7) 0%,transparent 100%),radial-gradient(1px 1px at 55% 20%,rgba(255,255,255,0.8) 0%,transparent 100%),radial-gradient(1px 1px at 75% 60%,rgba(255,255,255,0.6) 0%,transparent 100%),radial-gradient(1px 1px at 90% 35%,rgba(255,255,255,0.9) 0%,transparent 100%),radial-gradient(2px 2px at 20% 80%,rgba(100,180,255,0.8) 0%,transparent 100%),radial-gradient(2px 2px at 85% 15%,rgba(180,100,255,0.8) 0%,transparent 100%);z-index:0; }
.page { position:relative;z-index:10;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:1rem; }
.glass { background:rgba(255,255,255,0.07);backdrop-filter:blur(25px);border:1px solid rgba(255,255,255,0.15);border-radius:28px;padding:2.5rem 2rem;width:100%;max-width:420px;box-shadow:0 8px 40px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.15);position:relative; }
.glass::before { content:"";position:absolute;top:0;left:10%;right:10%;height:1px;background:linear-gradient(90deg,transparent,rgba(167,139,250,0.8),rgba(96,165,250,0.8),transparent); }
.logo { text-align:center;margin-bottom:1.5rem; }
.logo h1 { font-size:2rem;font-weight:900;margin:0.4rem 0 0;background:linear-gradient(135deg,#a78bfa,#60a5fa,#34d399);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text; }
...
.chat-header { text-align:center;padding:1rem 0 0.5rem; }
...
.stats { display:flex;justify-content:center;gap:1.5rem;padding:0.6rem 1rem;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05);border-radius:12px;margin:0.5rem 0; }
...
if not st.session_state.logged_in:
    if video_src:
        st.markdown(f'''<video class="vid-bg" autoplay muted loop playsinline>
        <source src="data:video/mp4;base64,{video_src}" type="video/mp4"></video>
        <div class="dark-overlay"></div>''', unsafe_allow_html=True)
    else:
